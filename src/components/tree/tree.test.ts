...
import type PTree from "./tree.component.js";
import type PTreeItem from "../tree-item/tree-item.js";
...
describe("<p-tree>", () => {
  ...
  beforeEach(async () => {
    el = await fixture(html`
      <p-tree>
        <p-tree-item>Node 1</p-tree-item>
        <p-tree-item>Node 2</p-tree-item>
        <p-tree-item id="expandable">
          Parent Node
          <p-tree-item>Child Node 1</p-tree-item>
          <p-tree-item>
            Child Node 2
            <p-tree-item>Child Node 2 - 1</p-tree-item>
            <p-tree-item>Child Node 2 - 2</p-tree-item>
          </p-tree-item>
        </p-tree-item>
        <p-tree-item>Node 3</p-tree-item>
      </p-tree>
    `);
  });
  ...
  it("should not focus collapsed nodes", async () => {
    // Arrange
    const parentNode = el.children[2] as PTreeItem;
    const childNode = parentNode.children[1] as PTreeItem;
  ...
  describe("Keyboard navigation", () => {
    ...
    describe("when Home is pressed", () => {
      it("should move the focus to the first tree item in the tree", async () => {
        // Arrange
        const parentNode = el.children[3] as PTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe("when End is pressed", () => {
      it("should move the focus to the last tree item in the tree", async () => {
        // Arrange
        const parentNode = el.children[0] as PTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe("when Enter is pressed", () => {
      describe('and selection is "single"', () => {
        it("should select only one tree item", async () => {
          // Arrange
          el.selection = "single";
          const node = el.children[1] as PTreeItem;
          node.focus();
          await el.updateComplete;
      ...
      describe('and selection is "leaf"', () => {
        it("should select only one tree item", async () => {
          // Arrange
          el.selection = "leaf";
          const node = el.children[0] as PTreeItem;
          node.focus();
          await el.updateComplete;
  ...
  describe("Interactions", () => {
    ...
    describe("when the user clicks on a tree item", () => {
      describe('and selection is "single"', () => {
        it("should select only one tree item", async () => {
          // Arrange
          el.selection = "single";
          const node0 = el.children[0] as PTreeItem;
          const node1 = el.children[1] as PTreeItem;
      ...
      describe('and selection is "multiple"', () => {
        ...
        it("should set the indeterminate state to tree items if a child is selected", async () => {
          ...
          const parentNode = el.children[2] as PTreeItem;
          const childNode = parentNode.children[0] as PTreeItem;
    ...
    describe('when selection is "single"', () => {
      describe("and user clicks on same item twice", () => {
        it("should emit `p-selection-change` event once", async () => {
          // Arrange
          el.selection = "single";
          await el.updateComplete;

          const selectedChangeSpy = sinon.spy();
          el.addEventListener("p-selection-change", selectedChangeSpy);
          ...
          expect(selectedChangeSpy.args[0][0]).to.deep.include({
  ...
  describe('when selection is "leaf"', () => {
    ...
    describe("and user clicks on expandable item", () => {
      it("should not emit `p-selection-change` event", async () => {
        // Arrange
        el.selection = "leaf";
        await el.updateComplete;
        ...
        const node = el.querySelector<PTreeItem>("#expandable")!;
  ...
  describe('when selection is "multiple"', () => {
    describe("and user clicks on same item twice", () => {
      it("should emit `p-selection-change` event twice", async () => {
        // Arrange
        el.selection = "multiple";
        await el.updateComplete;
        ...
        expect(selectedChangeSpy.args[1][0]).to.deep.include({
          detail: { selection: [] },
        });
      });
    });
      ...
      describe("and a parent node is selected", () => {
        it("should select all the nested children", async () => {
          ...
          const tree = await fixture<PTree>(html`
            <p-tree selection="multiple">
              <p-tree-item selected>
                Parent Node
                <p-tree-item selected>Child Node 1</p-tree-item>
                <p-tree-item>
                  Child Node 2
                  <p-tree-item>Child Node 2 - 1</p-tree-item>
                  <p-tree-item>Child Node 2 - 2</p-tree-item>
  ...
  it("Should not render 'null' if it can't find a custom icon", async () => {
    const tree = await fixture<PTree>(html`
      <p-tree>
        <p-tree-item>
          Item 1
          <p-icon name="1-circle" slot="expand-icon"></p-icon>
